...
func main() {
	var fromKeyName string
	var toAddr string
	var amountToSend int64
	var broadcast bool

	flag.StringVar(&fromKeyName, "k", "", "key resource name")
	...
	fromAddr, wif, err := GetAddrFromKey(fromKeyName, networkParams)
	if err != nil {
		fmt.Println(err)
	}
...
func GetAddrFromKey(fromAddrPrivKey string, networkParams *chaincfg.Params) (string, *btcutil.WIF, error) {
