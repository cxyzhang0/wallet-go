...
func opcodeCheckLockTimeVerify(op *parsedOpcode, vm *Engine) error {
	...
	err = verifyLockTime(int64(vm.tx.LockTime), tsx.LockTimeThreshold,
		int64(lockTime))
	if err != nil {
		return err
	}
...
func opcodeCheckSig(op *parsedOpcode, vm *Engine) error {
	...
	hashType := tsx.SigHashType(fullSigBytes[len(fullSigBytes)-1])
	...
	if vm.isWitnessVersionActive(0) {
		var sigHashes *tsx.TxSigHashes
		if vm.hashCache != nil {
			sigHashes = vm.hashCache
		} else {
			sigHashes = tsx.NewTxSigHashes(&vm.tx)
		}
...
func opcodeCheckMultiSig(op *parsedOpcode, vm *Engine) error {
	...
	if numPubKeys > tsx.MaxPubKeysPerMultiSig {
		str := fmt.Sprintf("too many pubkeys: %d > %d",
			numPubKeys, tsx.MaxPubKeysPerMultiSig)
		return scriptError(ErrInvalidPubKeyCount, str)
	}
	...
	for numSignatures > 0 {
		...
		hashType := tsx.SigHashType(rawSig[len(rawSig)-1])
