...
type Engine struct {
	...
	numOps          int
	flags           ScriptFlags
	sigCache        *tsx.SigCache
	hashCache       *tsx.TxSigHashes
...
func (vm *Engine) isBranchExecuting() bool {
	...
	return vm.condStack[len(vm.condStack)-1] == tsx.OpCondTrue
}
...
func (vm *Engine) executeOpcode(pop *parsedOpcode) error {
	...
	if pop.opcode.value > OP_16 {
		vm.numOps++
		if vm.numOps > tsx.MaxOpsPerScript {
			str := fmt.Sprintf("exceeded max operation limit of %d",
				tsx.MaxOpsPerScript)
			return scriptError(ErrTooManyOperations, str)
		}

	} else if len(pop.data) > tsx.MaxScriptElementSize {
		str := fmt.Sprintf("element size %d exceeds max allowed size %d",
			len(pop.data), tsx.MaxScriptElementSize)
...
func (vm *Engine) checkHashTypeEncoding(hashType tsx.SigHashType) error {
	if !vm.hasFlag(ScriptVerifyStrictEncoding) {
		return nil
	}

	sigHashType := hashType & ^tsx.SigHashAnyOneCanPay
	if sigHashType < tsx.SigHashAll || sigHashType > tsx.SigHashSingle {
...
func NewEngine(scriptPubKey []byte, tx *wire.MsgTx, txIdx int, flags ScriptFlags,
	sigCache *tsx.SigCache, hashCache *tsx.TxSigHashes, inputAmount int64) (*Engine, error) {
	...
	if vm.hasFlag(ScriptVerifySigPushOnly) && !tsx.IsPushOnlyScript(scriptSig) {
	...
	if vm.hasFlag(ScriptVerifyWitness) {
		...
		case len(tx.TxIn[txIdx].Witness) != 0 && vm.bip16:
			...
			if len(sigPops) == 1 && canonicalPush(sigPops[0]) &&
				tsx.IsWitnessProgram(sigPops[0].data) {
		...
		if witProgram != nil {
			var err error
			vm.witnessVersion, vm.witnessProgram, err = tsx.ExtractWitnessProgramInfo(witProgram)
			if err != nil {
				return nil, err
			}
		} else {
